...
void GLHelper::key_cb(GLFWwindow *pwin, int key, int scancode, int action, int mod) {
  ...
  else if (GLFW_RELEASE == action || GLFW_REPEAT == action)
  {
      keystateT = GL_FALSE;
      keystateM = GL_FALSE;
      keystateA = GL_FALSE;
...
void GLHelper::mousepos_cb(GLFWwindow *pwin, double xpos, double ypos) {
  ...
  mcn.x = static_cast<float>(xpos / static_cast<double>(GLHelper::width));
  mcn.y = static_cast<float>(1.0 - ypos / static_cast<double>(GLHelper::height));
}
...
void GLHelper::print_specs() {
    GLint major_ver{}, minor_ver{}, current_context{}, max_vertex, max_indices{}, max_tex{},
        max_gen_vertex_attri{}, max_vertex_buffer_bindings{}, max_tex_units{}, max_tex_units_vert{}, max_tex_units_frag{};
    ...
    std::cout << "Maximum number of generic vertex attributes: " << max_gen_vertex_attri << '\n';
    std::cout << "Maximum number of vertex buffer binding points: " << max_vertex_buffer_bindings << '\n';
    std::cout << "Max combined texture units: " << max_tex_units << '\n';
    std::cout << "Max texture units used by vertex shader: " << max_tex_units_vert << '\n';
    std::cout << "Max texture units used by fragment shader: " << max_tex_units_frag << '\n';
}