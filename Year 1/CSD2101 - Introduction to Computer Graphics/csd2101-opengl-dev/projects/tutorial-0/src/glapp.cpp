...
GLboolean GLApp::keystateR = GL_FALSE;
GLboolean GLApp::keystateG = GL_FALSE;
GLboolean GLApp::keystateB = GL_FALSE;
...
void GLApp::update() {
  ...
  else if (GLApp::keystateG == GL_TRUE)
  {
  ...
  else if (GLApp::keystateB == GL_TRUE)
  {
      // Set Blue background
      clear_color[0] = 0.0f;
      clear_color[1] = 0.0f;
...
void GLApp::key_cb(GLFWwindow *pwin, int key, int scancode, int action, int mod) {
  ...
  else if (GLFW_KEY_G == key && GLFW_PRESS == action)
  {
      keystateR = GL_FALSE;
      keystateG = GL_TRUE;
      keystateB = GL_FALSE;
