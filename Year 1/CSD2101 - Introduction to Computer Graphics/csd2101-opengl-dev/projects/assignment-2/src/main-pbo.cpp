...
int main(int argc, char* argv[]) {
  ...
  // Override the default dimension values with a width and height of 1000x1000 for assignment 2
  // Use the command line arguments -w and -h to specify custom window dimensions
  args.window_width = args.window_height = 1000;

  if (!args.parseArguments(argc, argv)) return 0;

  if (args.window_width != 1000 || args.window_height != 1000) {
      std::cout << "\n[INFORMATION]: This assignment considers a window where both dimensions are set to the minimum of the width and height.\n";
      args.window_width = args.window_height = std::min(args.window_width, args.window_height);
  }

  // Part 1
  init(args.window_width, args.window_height, "A2");
