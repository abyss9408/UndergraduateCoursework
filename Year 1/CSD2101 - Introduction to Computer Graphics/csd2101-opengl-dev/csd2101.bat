...
:MENU
	...
	IF %M%==8 GOTO CREATE_PROJECT_ASSIGNMENT_2
	IF %M%==100 (
		GOTO PLAYLOGS
	) ELSE (
...
:CREATE_PROJECT_TUTORIAL_6
	CALL :CHECK_OPENGL_LIBS "%lib-dir%"
	CALL :COPY_FROM_REMOTE_URL https://giraphics.github.io/csd-2101/tutorial-6.zip projects
	GOTO BUILDCMAKELISTS

:CREATE_PROJECT_ASSIGNMENT_1
	CALL :CHECK_OPENGL_LIBS "%lib-dir%"
	CALL :COPY_FROM_REMOTE_URL https://giraphics.github.io/csd-2101/assignment-1.zip projects
...
:CREATE_PROJECT_ASSIGNMENT_2
	CALL :CHECK_OPENGL_LIBS "%lib-dir%"
	CALL :COPY_FROM_REMOTE_URL https://giraphics.github.io/csd-2101/assignment-2.zip projects

	set "images-src-path=%projects-dir%/assignment-2/%images-resource-dir%"
    CALL :COPY_RESOURCE %images-src-path% %images-resource-dir%

	set "mesh-src-path=%projects-dir%/assignment-2/%meshes-resource-dir%"
    CALL :COPY_RESOURCE %mesh-src-path% %meshes-resource-dir%

	set "dpml-src-path=%projects-dir%/assignment-2/dpml"
    CALL :COPY_RESOURCE %dpml-src-path% %lib-dir%/dpml
...
:BUILDCMAKELISTS
	...
	(
		...
		ECHO include_directories(${CMAKE_CURRENT_LIST_DIR}/lib/dpml/include^)
		ECHO.
		...
		IF EXIST "%projects-dir%" (
		    ...
		    ECHO     PRIVATE
		    ECHO         ^$^<$^<CONFIG:Debug^>:${CMAKE_CURRENT_LIST_DIR}/lib/dpml/lib/Debug^>
		    ECHO         ^$^<$^<CONFIG:Release^>:${CMAKE_CURRENT_LIST_DIR}/lib/dpml/lib/Release^>
		    ECHO ^)
